...
def run_stage(paths: Dict[str, Path], cfg: Dict[str, Any]) -> None:
    ...
    if out_path.exists() and not cfg.get("force", False):
        logger.info(f"[contexts] Есть готовый файл, пропускаю: {out_path.name}")
        return
    ...
    # Построим кеш сцен: (chapter, scene) -> { text, sentences, offsets }
    ...
    entities = coref_data.get("entities", [])
    total_entities = len(entities)
    logger.info(f"[contexts] Всего персонажей из coref: {total_entities}")

    results: List[Dict[str, Any]] = []
    # Для каждого персонажа собираем список событий (contexts)
    for ent in entities:
        ent_id    = ent.get("id")
        norm      = ent.get("norm", "")
        ...
        for m in mentions:
            ...
            # текстовый фрагмент ограниченной длины
        ...
        logger.info(f"[contexts] Персонаж id={ent_id} ({norm}) → собрано {len(events)} контекстов")
        results.append({
            "entity_id": ent_id,
            "norm":      norm,
            "gender":    gender,
